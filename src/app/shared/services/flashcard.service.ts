...
import * as flashcardsJson from '../../assets/flashcards.json'; // Importar el JSON
...
export class FlashcardService {
  ...
  flashcards = signal<Flashcard[]>(flashcardsJson.flashcards); // Inicializa el signal con el array de flashcards
  ...
  currentQuestion = computed(
    () => this.flashcards()[this.currentCardIndex()]?.question
  ); // Optional chaining (?.)
  currentAnswer = computed(
    () => this.flashcards()[this.currentCardIndex()]?.answer
  ); // Optional chaining (?.)
  ...
  nextCard() {
    this.currentCardIndex.update((index) =>
